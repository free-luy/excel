...
public class ImportExcel {
	...
	public ImportExcel(MultipartFile multipartFile, int headerNum,
			int sheetIndex,boolean b) throws InvalidFormatException, IOException {
		this(multipartFile.getOriginalFilename(), multipartFile, headerNum, sheetIndex);
	}
	...
	public ImportExcel(String fileName, MultipartFile multipartFile,
			int headerNum, int sheetIndex) throws IOException {
		if (StringUtils.isBlank(fileName)) {
			throw new RuntimeException("导入文档为空!");
		} else if (fileName.toLowerCase().endsWith("xls")||fileName.toLowerCase().endsWith("xlsx")) {
			//Workbook book = null;
	        try {
	        	this.wb = new XSSFWorkbook(multipartFile.getInputStream());
	        } catch (Exception ex) {
	        	
	        	this.wb = new HSSFWorkbook(multipartFile.getInputStream());
	        }
			//this.wb = new HSSFWorkbook(is);
		} else {
			throw new RuntimeException("文档格式不正确!");
	...
	public <E> boolean validHeadText(Class<E> cls, int... groups) {
		boolean b = true;
		...
		for (Object[] os : annotationList) {
			Object val = this.getCellValue(headRow, column++);
			if (val != null) {
				ExcelField ef = (ExcelField) os[0];
				// If is dict type, get dict value
				if (StringUtils.isNotBlank(ef.title())) {
